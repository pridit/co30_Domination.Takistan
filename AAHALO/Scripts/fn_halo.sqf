...
if (typename _this == typename objnull) then {
    ...
    //--- Init
    _dir = [[0,0,0],velocity _unit] call FUNC(BDirTo);
    _unit setdir _dir;
    [QGVAR(nswm), [_unit, "HaloFreeFall_non"]] call FUNC(NetCallEvent);
    ...
    if (_unit == player) then {
        ...
        bis_fnc_halo_action = _unit addaction [localize "STR_HALO_OPEN_CHUTE","AAHALO\Scripts\fn_HALO.sqf",[],1,true,true,"Eject"];
        ...
        0 spawn {
            scriptName "spawn_HaloFly";
            private ["_vAnim","_hAnim","_anim","_fpsCoef","_time","_dir","_vel","_v","_h","_vLimit","_hLimit"];
            _time = time - 0.1;
            while {alive player && {vehicle player == player} && {!__pGetVar(xr_pluncon)}} do {
                //--- FPS counter
                ...
                bis_fnc_halo_ppRadialBlur ppEffectCommit 0.01;
                hintSilent format [(localize "STR_DOM_MISSIONSTRING_67"), round (position player select 2)];
                sleep 0.01;
            };
            //--- End
            hintSilent "";
            ...
            if (!alive player) then {
                __TRACE("adthppnemstpsraswrfldnon_1 net event");
                [QGVAR(nswm), [player, "adthppnemstpsraswrfldnon_1"]] call FUNC(NetCallEvent);
        ...
        while {(position _unit select 2) > 100} do {
            ...
            if (_destination distance [position _unit select 0,position _unit select 1,0] > 10) then {
                _vel = velocity _unit;
                _dirTo = [_unit,_destination] call FUNC(BDirTo);
...
if (typename _this == typename []) then {
    ...
    if (_unit == player) then {
        ...
        waituntil {(position (vehicle player) select 2) < 1.5 || {!alive player} || {__pGetVar(xr_pluncon)}};
        hintSilent "";
        ...
        if (!alive player || {__pGetVar(xr_pluncon)}) then {
            ...
            [QGVAR(nswm), [player, ""]] call FUNC(NetCallEvent);
        };
    };
};
