...
PARAMS_6(_man,_target_pos,_rad,_bla,_chuto,_is_ammo);
...
while {alive _chuto && {(getPos _chuto select 2) > 5}} do {
    ...
    if (!_is_ammo && {!_detached}) then {
        if (position _man select 2 <= 4) then {
            ...
            _helper1 = GVAR(HeliHEmpty) createVehicleLocal [_pos_man select 0, _pos_man select 1, 0];
...
deleteVehicle _cone;
if (_is_ammo) then {
    [QGVAR(air_box), _pos_conex] call FUNC(NetCallEventToClients);
} else {
