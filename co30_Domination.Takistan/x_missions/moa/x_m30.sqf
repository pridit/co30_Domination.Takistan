        ...
        if (_time_over > 0) then {
            if (_time_over == 3) then {
                if (_endtime - time <= 600) then {
                    _time_over = 2;
#ifndef __TT__
                    [QGVAR(kbmsg), [29]] call FUNC(NetCallEventCTS);
#else
                    [QGVAR(kbmsg), [30]] call FUNC(NetCallEventCTS);
#endif
                ...
                if (_time_over == 2) then {
                    if (_endtime - time <= 300) then {
                        _time_over = 1;
#ifndef __TT__
                        [QGVAR(kbmsg), [31]] call FUNC(NetCallEventCTS);
#else
                        [QGVAR(kbmsg), [32]] call FUNC(NetCallEventCTS);
#endif
                    };
                } else {
                    if (_time_over == 1) then {
                        if (_endtime - time <= 120) then {
                            _time_over = 0;
#ifndef __TT__
                            [QGVAR(kbmsg), [33]] call FUNC(NetCallEventCTS);
#else
                            [QGVAR(kbmsg), [34]] call FUNC(NetCallEventCTS);
#endif
                        };
                    };
                };
