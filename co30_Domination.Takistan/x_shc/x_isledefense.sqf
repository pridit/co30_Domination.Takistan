...
_make_isle_grp = {
    ...
    #ifndef __TT__
    while {(count _start_point < 3 || {(_start_point distance GVAR(FLAG_BASE) < 2000)} || {(_start_point distance _old_old_start < 500)}) && {_dc < 99}} do {
    #else
    while {(count _start_point == 0 || {(_start_point distance GVAR(WFLAG_BASE) < 1500)} || {(_start_point distance GVAR(EFLAG_BASE) < 1500)} || {_start_point distance _old_old_start < 500}) && {_dc < 99}} do {
    #endif
        _start_point = [GVAR(with_isledefense) select 0,GVAR(with_isledefense) select 1,GVAR(with_isledefense) select 2,GVAR(with_isledefense) select 3] call FUNC(GetRanPointSquare);
        sleep 0.353;
        __INC(_dc);
