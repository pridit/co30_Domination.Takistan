...
if (isNil "KEGs_FuncsInited") then {
    ...
    call compile preprocessFileLineNumbers "spect\spect_funcs.sqf";
    spectate_events = compile preprocessFileLineNumbers "spect\specta_events.sqf";
    KEGs_menucamslbchange = compile preprocessFileLineNumbers "spect\menucamslbchange.sqf";
};
...
_deadstr = (localize "STR_DOM_MISSIONSTRING_90");
_unknownstr = "Unknown";
_name = _unknownstr;
_pilotstr = (localize "STR_DOM_MISSIONSTRING_91");
_driverstr = (localize "STR_DOM_MISSIONSTRING_92");
_gunnerstr = (localize "STR_DOM_MISSIONSTRING_93");
_commanderstr = (localize "STR_DOM_MISSIONSTRING_94");
_selcamstr = (localize "STR_DOM_MISSIONSTRING_95");
...
_cameraNames = [(localize "STR_DOM_MISSIONSTRING_122"), (localize "STR_DOM_MISSIONSTRING_123"), (localize "STR_DOM_MISSIONSTRING_124"), (localize "STR_DOM_MISSIONSTRING_125"), (localize "STR_DOM_MISSIONSTRING_126")];
_dummy = d_HeliHEmpty createVehicleLocal [0, 0, 0];
...
_cLbMissileCam = lbAdd [_cLBCameras, (localize "STR_DOM_MISSIONSTRING_127")];
_cLbToggleNVG = lbAdd [_cLBCameras, (localize "STR_DOM_MISSIONSTRING_128")];
_cLbToggleTags = lbAdd [_cLBCameras, (localize "STR_DOM_MISSIONSTRING_129")];
_cLbToggleAiFilter = lbAdd [_cLBCameras, (localize "STR_DOM_MISSIONSTRING_130")];
_cLbToggleDeadFilter = lbAdd [_cLBCameras, (localize "STR_DOM_MISSIONSTRING_131")];
...
0 spawn {
    scriptName "spawn_SpectMessage";
    sleep 3;
    if (dialog) then {
        if (!KEGs_can_exit_spectator) then {
            cutText[(localize "STR_DOM_MISSIONSTRING_132"), "PLAIN DOWN", 0.75];
        } else {
            cutText[(localize "STR_DOM_MISSIONSTRING_133"), "PLAIN DOWN", 0.75];
        };
    };
};
...
while {dialog} do {
    ...
    if (!KEGs_Spect_Init) then {if (count KEGs_deathCam > 0) then {player groupChat (localize "STR_DOM_MISSIONSTRING_134"); KEGs_Spect_Init = true}};
...
if (KEGs_no_butterfly_mode) exitWith {
    ...
    0 spawn {sleep 0.01; [_player, _killer, "noWait"] execVM "spect\specta.sqf"};
};
...
cutText [(localize "STR_DOM_MISSIONSTRING_135"), "PLAIN DOWN", 0.75];
...
deletevehicle _bird;
KEGs_SPECTATINGON = nil;
[_player, _killer, "noWait"] execVM "spect\specta.sqf";