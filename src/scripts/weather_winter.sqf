...
while {true} do {
    if (!_enabled) then {
        if (__XJIPGetVar(GVAR(winterw)) > 0) then {
            _enabled = true;
            setWind [2.342, 3.108, true];
            _pphandle ppEffectEnable true;
            _pphandle ppEffectCommit 3;
            if (__XJIPGetVar(GVAR(winterw)) == 1) then {
        ...
        if (__XJIPGetVar(GVAR(winterw)) == 0) then {
            ...
            if (!_snowon && {__XJIPGetVar(GVAR(winterw)) == 1}) then {
                _snow setDropInterval 0.01;
                _fog setDropInterval 0.01;
                _snowon = true;
            } else {
                if (_snowon && {__XJIPGetVar(GVAR(winterw)) == 2}) then {
                    _snowon = false;
                    _fog setDropInterval 0;
                    _snow setDropInterval 0;
                };
