...
_delveccrew = {
    scriptName "spawn_x_createparas_delveccrew";
...
_make_jump = {
    scriptName "spawn_x_createparas_make_jump";
    ...
    if (!isNull _vehicle && {alive _vehicle} && {alive _driver_vec} && {canMove _vehicle}) then {
        ...
        if (_vehicle distance _current_target_pos < 500) then {
...
#ifdef __TT__
                [_one_unit, {_this addMPEventHandler ["MPkilled", {if (isServer) then {[1,_this select 1] call FUNC(AddKills)}}]}] call FUNC(setUnitCode);
#endif
            ...
            [_paragrp, _current_target_pos, _cur_radius] spawn {
                scriptName "spawn_x_createparas_makeuse";
...
#ifdef __TT__
    _vehicle addMPEventHandler ["MPkilled", {if (isServer) then {[8,_this select 1] call FUNC(AddKills)}}];
#else
