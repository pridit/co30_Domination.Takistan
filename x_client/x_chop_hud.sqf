...
#include "x_setup.sqf"
private ["_state", "_vec", "_hud", "_control", "_chopttype", "_hudoff", "_nobjects", "_nearest", "_dummy", "_chdispx", "_nx", "_ny", "_px", "_py", "_control2", "_angle", "_a", "_b", "_dif", "_chdispx2", "_controlxx","_distvstr", "_helper_h", "_doexitit"];
...
if (GVAR(show_chopper_welcome)) then {
    ...
    if (isNil QUOTE(FUNC(X_Chopper_Welcome))) then {
        FUNC(X_Chopper_Welcome) = {
            scriptName "func_X_Chopper_Welcome";
        ...
        if ((_vec isKindOf "Helicopter" || {typeOf _vec == "MV22"}) && {!(_vec isKindOf "ParachuteBase")} && {!(_vec isKindOf "BIS_Steerable_Parachute")}) then {
            ...
            if (GVAR(show_chopper_welcome)) then {
                if (_chopttype > -1) then {
                    [_chopttype,_vec] spawn FUNC(X_Chopper_Welcome);
                    sleep 0.321;
            ...
            if (_chopttype in [0,1]) then {
                ...
                while {GVAR(player_in_vec) && {alive player} && {player == driver _vec} && {!__pGetVar(xr_pluncon)}} do {
                    sleep 0.421;
                };
                _doexitit = true;
                67322 cutRsc ["Default", "PLAIN"];
