        ...
        if (!(GV(_vehicle,GVAR(Vehicle_Attached))) && {(_pos select 2 > 2.5)} && {(_pos select 2 < 11)}) then {
            ...
            if (!isNull _nearest) then {
                if (
                    ...
                    (speed _nearest > 10) ||
                    {(position _nearest select 2 > 5)} ||
                    {!((toUpper (typeof _nearest)) in _possible_types) && !((typeof _nearest) in _possible_types)}
                ) then {
                    _nearest = objNull;
                };
            };
...
//#endif
                    ...
                    [QGVAR(engineon), [_nearest, false]] call FUNC(NetCallEventSTO);
