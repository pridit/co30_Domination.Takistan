...
if (_arg == 0 && {(_vec call d_fnc_isVecLocked)}) exitWith {(localize "STR_DOM_MISSIONSTRING_275") call FUNC(GlobalChat)};

if (_arg == 1 && {!(_vec call d_fnc_isVecLocked)}) exitWith {(localize "STR_DOM_MISSIONSTRING_276") call FUNC(GlobalChat)};

_depl = GV(_vec,GVAR(MHQ_Deployed));
if (!isNil "_depl" && {_depl} && {_arg == 1}) exitWith {
