...
#include "x_setup.sqf"
private ["_speed_str", "_fuel_str", "_dam_str", "_dir_str", "_gendirlist", "_vec", "_welcome_str", "_vec_msg1", "_struct_text", "_endtime", "_type_name", "_vec_string", "_vdir", "_gendir", "_dstr", "_count", "_control", "_type_weap", "_dirtmp", "_fuelcap"];
...
if (isNil QUOTE(FUNC(vec_welcome_message))) then {
    FUNC(vec_welcome_message) = {
        scriptName "func_vec_welcome_message";
        private ["_vec","_vtype"];
        ...
        while {vehicle player != player && {alive player} && {time < _endtime} && {!__pGetVar(xr_pluncon)}} do {
            sleep 1;
            hintSilent _struct_text;
        };
        hintSilent "";
    ...
    if (GVAR(show_vehicle_welcome) == 0) then {
        ...
        if (!isNil "_vtype") then {
            if (_vtype == "MHQ" || {_vtype == "Engineer"}) then {
                [_vec, _vtype] spawn FUNC(vec_welcome_message);
            };
        };
    };
    ...
    if (_vec isKindOf "StaticWeapon" && {!(_vec isKindOf "ACE_SpottingScope")} && {!(_vec isKindOf "StaticATWeapon")}) then {
        while {GVAR(player_in_vec) && {alive player} && {!__pGetVar(xr_pluncon)}} do {
            if (player == gunner _vec) then {
                ...
                _disp = __uiGetVar(DVEC_HUD);
                _ison = true;
                while {GVAR(player_in_vec) && {alive player} && {player == gunner _vec}} do {
                    _crewctrl = __uiGetVar(GVAR(rscCrewText)) displayCtrl 9999;
                    _cctrlshown = if (isNil "_crewctrl") then {false} else {ctrlShown _crewctrl};
                    if (!visibleMap && {!_cctrlshown}) then {
                        if (!_ison) then {
