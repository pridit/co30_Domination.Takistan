...
if (GVAR(without_nvg) == 0 && {_pilot1 hasWeapon "NVGoggles"}) then {_pilot1 removeWeapon "NVGoggles"};
_pilot1 setVariable ["BIS_noCoreConversations", true];

_pilot2 = _owngroup createUnit [GVAR(sm_pilottype), position _pilot1, [], 60, "FORM"];
if (GVAR(without_nvg) == 0 && {_pilot2 hasWeapon "NVGoggles"}) then {_pilot2 removeWeapon "NVGoggles"};
    ...
    if (_time_over > 0) then {
        if (_time_over == 3) then {
            if (_endtime - time <= 600) then {
                _time_over = 2;
#ifndef __TT__
                [QGVAR(kbmsg), [23]] call FUNC(NetCallEventCTS);
#else
                [QGVAR(kbmsg), [24]] call FUNC(NetCallEventCTS);
#endif
            };
        } else {
            if (_time_over == 2) then {
                if (_endtime - time <= 300) then {
                    _time_over = 1;
#ifndef __TT__
                    [QGVAR(kbmsg), [25]] call FUNC(NetCallEventCTS);
#else
                    [QGVAR(kbmsg), [26]] call FUNC(NetCallEventCTS);
#endif
                };
            } else {
                if (_time_over == 1) then {
                    if (_endtime - time <= 120) then {
                        _time_over = 0;
#ifndef __TT__
                        [QGVAR(kbmsg), [27]] call FUNC(NetCallEventCTS);
#else
                        [QGVAR(kbmsg), [28]] call FUNC(NetCallEventCTS);
#endif
                    };
                };
            };
